...
const ProductSelected: React.FC = () => {
  ...
  const [seletedImage, setSeletedImage] = useState<string>(
    product.images.mainImage
  );
  const [seletedColor, setSeletedColor] = useState<string>("");
  const [selectedSize, setSelectedSize] = useState<string>("");

  const { products, handleBuy, quantities, addQuantity, decreaseQuantity } =
    useCarrinho();
        ...
        <>
          <section className="bg-pink_fundo pl-20 flex gap-4 items-center w-full h-24">
            <div className="flex items-center gap-3.5 py-2 pr-4 h-fit border-r border-black text-zinc-400">
              Home <i className="fas fa-chevron-right text-black"></i> Shop{" "}
            ...
            <div className="flex justify-center md:flex-row flex-col md:gap-6">
              <div className="flex gap-10 w-5/12">
                    ...
                    <ul>
                      <li key={image}>
                        <img
                          ...
                          alt="Miniatura móvel"
                          className={
                            "w-24 h-24 bg-orange-100 object-cover cursor-pointer"
                          }
                ...
                <img
                  src={seletedImage}
                  alt="Sofá Asgaard"
                  className="md:w-96 md:h-[500px] w-80 h-[400px]"
                />
              </div>
                ...
                <div className="flex flex-col gap-4 mb-8 mt-4">
                  <div className="flex flex-col gap-3">
                    ...
                    <ul className="flex gap-3">
                      ...
                      product.sizes[0].width &&
                      product.sizes[0].depth ? (
                        <>
                          <li>
                            <button
                          ...
                          <li>
                            <button
                              className={`py-2 px-4 rounded-lg text-center ${
                                selectedSize === product.sizes[0].width
                                  ? "bg-yellow-700 text-white"
                                  : "bg-orange-100 text-black"
                              }`}
                              onClick={() =>
                                handleSizeClick(product.sizes[0].width)
                              }
                            >
                          ...
                          <li>
                            <button
                              className={`py-2 px-4 rounded-lg text-center ${
                                selectedSize === product.sizes[0].depth
                                  ? "bg-yellow-700 text-white"
                                  : "bg-orange-100 text-black"
                              }`}
                              onClick={() =>
                                handleSizeClick(product.sizes[0].depth)
                              }
                            >
                ...
                <div className="flex items-center gap-4 mb-8 ">
                  ...
                  seletedColor !== "" ? (
                    <button
                      ...
                      className="border border-zinc-400 rounded-lg flex items-center gap-4 py-2 px-3.5"
                    >
                      Add To Cart
                    </button>
                  ) : (
                    <button
                      onClick={() => warning()}
                      className="border border-zinc-400 rounded-lg flex cursor-not-allowed items-center gap-4 py-2 px-3.5"
                    >
                      Add To Cart
                    </button>
                  )}
          ...
          <section className="border-t mb-10 border-zinc-300">
            <div className="flex justify-center my-10 gap-6">
              <h1
                onClick={() => setClickDescription(true)}
                className={
                  clickDescription
                    ? "md:text-xl text-lg font-medium cursor-pointer"
                    : "md:text-xl text-lg text-zinc-400 font-medium cursor-pointer"
                }
              >
                Description
              </h1>
              <h1
                onClick={() => setClickDescription(!clickDescription)}
                className={
                  clickDescription
                    ? "md:text-xl text-lg text-zinc-400 font-medium cursor-pointer"
                    : "md:text-xl text-lg  font-medium cursor-pointer"
              ...
              <h1 className="md:text-xl text-lg text-zinc-400 font-medium cursor-pointer">
              ...
              {clickDescription ? (
                ...
                <>
                  <textarea
                    ...
                    className="w-3/5 h-full pt-4 pl-6 text-black border-2 boder-neutral-900 rounded-md focus:outline-none"
                    id="text"
                  ></textarea>
                </>
              )}
            ...
            <div className="flex md:flex-row flex-col items-center justify-center mt-8 gap-20">
              {product.images.gallery.slice(0, 2).map((image: string) => (
                <img
                  className="bg-pink_fundo h-72 w-96"
                  src={image}
                  alt="Imagem móvel"
                />
              ))}
